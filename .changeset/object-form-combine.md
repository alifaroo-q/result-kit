...
The guard is **element-wise, not hole-specific**: any array element that is not readable as a `Result` gets the same named diagnostic, where before a non-object element was read as a falsy `.ok` and returned *as if it were an `Err`*. Both are reachable only by casting past the signature. Relatedly, a non-array **iterable** (a `Set<Result>`, again cast-only) is now rejected with `expected an array or a record of Results, but received a non-array iterable` rather than being read as an empty record — §5.4 ships no iterable overload, and declining one silently was the worse of the two.
