...
**matchType**:
The expression form of `switch (error.type)`: an object literal of **arms**, one per variant of a **TypedError** union, each receiving its own narrowed variant, returning the union of the arms' return types. **Exhaustive by construction** — a missing arm is a compile error, and so is an arm keyed on a tag the union does not have. A catch-all is a **third parameter**, never a `_` key in the arm bag, because only that position can narrow it to the **residual** variants (the ones left unhandled) — and residual narrowing is the reason to prefer this over `switch` at all. Operates on the error, not on the **Result**: compose it under **match**'s `err` branch. Exhaustiveness is a property of a *closed* tag union, so a bare `TypedError` cannot be exhausted.
_Avoid_: match (that name is taken by the **Result** terminal, §5.3), matchError, matchTag, exhaustive, `.with().exhaustive()` (ts-pattern's chained form, which this deliberately is not), "the default arm" for the fallback — it is not a `default:` branch and does not see the whole union.
...
**keyed error**:
An accumulated error paired with the record key it came from — the shape **combineWithAllErrors** accumulates when its input is a **record** rather than an array. The key and the error stay **correlated** per entry, so switching on the key narrows the error to that key's variant; over a record with no literal keys it degenerates to a plain string key. Deliberately *not* the array form's shape: an array input still accumulates flat, because a position is already an attribution and a record's key is not otherwise recoverable. The price of the split is that a keyed error is not what **groupByType** and **prettifyErrors** read — those take the flat array, and the record form reaches them by mapping the errors out.
_Avoid_: tagged error (too close to **TypedError**, and it is Effect's word for a different thing), keyed result, error entry, "the record form's errors" as if the *type* were record-shaped — the accumulation is an array whose entries carry keys, not a record of errors, which is the alternative that was rejected.
...
- **matchType** consumes one **TypedError**; **match** consumes one **Result**. The idiomatic pairing is `match(r, { ok: …, err: (e) => matchType(e, …) })` — one leaves the **Result**, the other dispatches on the error inside it.
- **combineWithAllErrors** produces, *from an array input*, the accumulated `TypedError[]` that **groupByType** and **prettifyErrors** consume, and is their motivating source. It is not the only one: `partition`'s second half is also an error array, and is equally valid input. From a **record** input it accumulates **keyed errors** instead, which the two formatters do not accept — `errors.map((e) => e.error)` is the way across.
